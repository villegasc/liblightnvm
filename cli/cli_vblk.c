...
int _vblk_erase(struct nvm_cli *cli, struct nvm_vblk *vblk)
{
	ssize_t res = 0;
...
int _vblk_write(struct nvm_cli *cli, struct nvm_vblk *vblk)
{
	...
	nvm_cli_timer_stop();
	nvm_cli_timer_pr("nvm_buf_from_file");
...
int _vblk_read(struct nvm_cli *cli, struct nvm_vblk *vblk)
{
	const struct nvm_geo *geo = nvm_dev_get_geo(cli->args.dev);
	const size_t nbytes = nvm_vblk_get_nbytes(vblk);
	char *buf = NULL;
...
int _vblk_pad(struct nvm_cli *cli, struct nvm_vblk *vblk)
{
	ssize_t res = 0;
